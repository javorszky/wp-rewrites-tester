...
RUN set -ex \
    ...
    && apt-get update
RUN apt-get install --no-install-recommends --no-install-suggests -y ca-certificates git build-essential libssl-dev libpcre2-dev curl pkg-config \
...
RUN apt-mark showmanual | xargs apt-mark auto > /dev/null \
